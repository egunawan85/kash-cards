...
{
    public partial class cardsold : System.Web.UI.Page
    {
        CardService us = new CardService();

        protected void Page_Load(object sender, EventArgs e)
        {
            Master.setTitle("Card Sold");
            if (Common.checkID())
            {
        ...
        void bindGV()
        {
            ...
            if (op.Status == "success")
            {
                ...
                if (dt.Count > 0)
                {
                    ...
                    {
                        dt[i].Param1 = dt[i].Price.ToString() + " " + dt[i].Currency;
                        dt[i].Param2 = dt[i].InitialDeposit.ToString() + " " + dt[i].Currency;
