...
{
    public partial class usercommission : System.Web.UI.Page
    {
        ...
        {
            Master.setTitle("User Commission");
            if (Common.checkID())
            {
        ...
        void bindGV()
        {
            ...
            List<UserCommissionModel> dt;
            OutputModel op = new OutputModel();
            op = us.getUserComm();
            if (op.Status == "success")
            {
                dt = JsonConvert.DeserializeObject<List<UserCommissionModel>>(op.Data.ToString());
                if (dt.Count > 0)
                {
                    for (int i = 0; i < dt.Count; i++)
                    {
                        dt[i].Param1 = dt[i].Commission + "%";
                    }
        ...
        protected void btnUpdateComm_ServerClick(object sender, EventArgs e)
        {
            HtmlButton btn = (HtmlButton)sender;
            GridViewRow row = (GridViewRow)btn.NamingContainer;
            hfTID.Value = ((HiddenField)row.FindControl("hfID")).Value;
            hfCID.Value = ((HiddenField)row.FindControl("hfUID")).Value;
            txtUserIDUpdate.Text = hfCID.Value;
            txtEmailUpdate.Text = ((HiddenField)row.FindControl("hfEmail")).Value;
            txtCurrentFee.Text = ((HiddenField)row.FindControl("hfCommission")).Value + "%";
            txtNewFee.Text = ((HiddenField)row.FindControl("hfCommission")).Value;

            ScriptManager.RegisterClientScriptBlock(this, GetType(), "Pop", "var isModalComm = true", true);
            //getCardDetail(false, hfTID.Value);
        }

        protected void btnUpdateCommExec_Click(object sender, EventArgs e)
        {
            if (SessionLib.Current.Role == RoleModel.Approver || SessionLib.Current.Role == RoleModel.Viewer || SessionLib.Current.Role == RoleModel.Signer)
            {
                lblFailedComm.Text = "You have no authorize to do this request";
                divfailedcomm.Visible = true;
                ScriptManager.RegisterClientScriptBlock(this, GetType(), "Pop", "var isModalComm = true", true);
                return;
            }
            if (txtNewFee.Text == "")
            {
                lblFailedAdd.Text = "New commission rate cannot be empty";
                divfailedcomm.Visible = true;
                ScriptManager.RegisterClientScriptBlock(this, GetType(), "Pop", "var isModalComm = true", true);
                return;
            }
            ...
            UserCommissionModel uw = new UserCommissionModel();
            uw.ID = Convert.ToInt32(hfTID.Value);
            uw.UserID = hfCID.Value;
            uw.Commission = Convert.ToDouble(txtNewFee.Text);
        ...
        protected void btndfailedcomm_ServerClick(object sender, EventArgs e)
        {
            divfailedcomm.Visible = false;
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "Pop", "var isModalComm = true", true);
        }
    }
}