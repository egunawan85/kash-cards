...
{
    public partial class deposithistory : System.Web.UI.Page
    {
        CardService us = new CardService();

        protected void Page_Load(object sender, EventArgs e)
        {
            Master.setTitle("Card Deposit History");
            if (Common.checkID())
            {
        ...
        protected Boolean checkCreated(string x)
        {
            if (x == "created")
                return true;
            else
        ...
        protected Boolean checkCancelled(string x)
        {
            if (x == "cancelled")
                return true;
            else
        ...
        protected Boolean checkExpired(string x)
        {
            if (x == "expired")
                return true;
            else
        ...
        protected Boolean checkSuccess(string x)
        {
            if (x == "success")
                return true;
            else
        ...
        void bindGV()
        {
            ...
            List<DepositModel> dt;
            OutputModel op = new OutputModel();
            DepositFilterModel filterModel = new DepositFilterModel();
            filterModel.Status = "all";
            filterModel.CardNo = "all";
            op = us.getDepositTtransaction(filterModel);
            if (op.Status == "success")
            {
                dt = JsonConvert.DeserializeObject<List<DepositModel>>(op.Data.ToString());
                if (dt.Count > 0)
                {
                    for (int i = 0; i < dt.Count; i++)
                    {
                        dt[i].Param1 = dt[i].Amount.ToString() + " " + dt[i].Currency;
                        dt[i].Param2 = dt[i].Fee.ToString() + " " + dt[i].Currency;
                        dt[i].Param3 = dt[i].Total.ToString() + " " + dt[i].Currency;
                    }
