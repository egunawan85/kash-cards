...
| **P7** — Prepaid balance (deposit → reusable wallet balance → spend on cards; reconciliation sweep) | ✅ | core + sweep shipped (#16/#19/#26/#29/#31); dev-shakeout items remain → `07` |
| **P8** — Dashboard wiring & NewDesign redesign (FE↔BE fixes, sidebar/Settings, wallet panel + pay-from-balance) | ✅ | shipped to `main` (#37–#49); per-card-type art (DD-7) delivered under Plan 9 (#54) → `08` |
| **P9** — Pre-cutover WasabiCard sandbox polish (URL-Required, webhook-verify, dev test-credit tool, card art, UX sweep) | 🟡 | S1 #53 · S2 reconciled #57 · S3 #52 · S4 #54 · S6 #56 merged; **S5 wallet money-path verify in progress** → `09` |
| **P10** — Referral commission (default 10% of the platform **fee** → referrer's wallet; loss-proof cap, per-order dedup) | ✅ | shipped (#64) + deploy/docs wiring (#66); external red-team (Opus + Sonnet) clean → `10` |
