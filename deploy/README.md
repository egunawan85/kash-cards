...
deploy/
  ...
  config/                      # per-env infra config, selected by ENV (filled files GITIGNORED)
    ...
    .env.provision.prod.example#   prod infra (rg-kash-shared + rg-kash-prd, kv-kash-prd, named tunnel)
    .env.cloudflare.dev.example#   dev Cloudflare named-tunnel creds + ROUTES (host -> loopback)
    .env.cloudflare.prod.example#  prod Cloudflare creds + ROUTES (app/admin/api/... no -dev)
  ...
  secrets/            # operator-authored, GITIGNORED (templates committed as *.example)
    .env.example      #   TEMPLATE -> copy to .env.<env> (.env.dev, .env.prod): non-secret config
...
| Kind | dev | prod |
|------|-----|------|
| infra config | `config/.env.provision.dev` | `config/.env.provision.prod` |
| Cloudflare | `config/.env.cloudflare.dev` | `config/.env.cloudflare.prod` |
| non-secret app config | `secrets/.env.dev` | `secrets/.env.prod` |
| app secrets | `secrets/.vault.dev` | `secrets/.vault.prod` |
...
missing `.vault.prod` can never silently seed dev values into the prod Key Vault.
...
```bash
cp deploy/config/.env.provision.prod.example  deploy/config/.env.provision.prod    # fill blanks
cp deploy/config/.env.cloudflare.prod.example deploy/config/.env.cloudflare.prod   # fill CF token + ROUTES
cp deploy/secrets/.env.example  deploy/secrets/.env.prod                            # fill prod config
cp deploy/secrets/.vault.example deploy/secrets/.vault.prod                         # fill ROTATED prod secrets
ENV=prod ./deploy/provision-and-bootstrap.sh --with-deploy
# subsequent app redeploys: ENV=prod ./deploy/deploy.sh update
```
...
`.env.provision.prod` and `PUBLIC_BASE_URL=https://app.kash.cards` in `.env.prod`,
re-seed the prod KV, re-run `cloudflare-setup.sh`, then `ENV=prod ./deploy/deploy.sh update`.
