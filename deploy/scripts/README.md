...
| Path | Purpose |
|---|---|
| `load-env.ps1` | Local dev: load `secrets/.env.<env>` + `.vault.<env>` into the current process env (`-Env`, default `dev`) |
...
| `secrets/` | `seed-kv-secrets.sh` (upload per-env `.vault.<env>` / `.env.<env>` → Key Vault; `ENV` selects) |
