...
-- External RT (Opus, LOW): wrap the delete+reinsert in a transaction so a mid-script
-- failure cannot leave a partially-seeded smoke identity (the old DevSeed used a tx).
SET XACT_ABORT ON;
BEGIN TRANSACTION;
...
-- External RT (Sonnet, LOW): do NOT echo the API key -- run-command stdout is retained in
-- the Azure control plane. The key alone can't authenticate (it needs the wire secret too),
-- but keep credential-adjacent identifiers out of the audit trail.
PRINT 'seed-smoke-user: re-seeded $(SMOKE_EMAIL)';
