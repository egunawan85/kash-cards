...
namespace QryptoCard.API.Callback.Controllers.v1
{
    ...
    public class CallbackV1Controller : ApiController
    {
        ...
        // Scheduled deposit-into-card pump tick (forward covered intents to WasabiCard + confirm the
        // float landing). Defended exactly like monitor/balance: reject anything proxied through
        // Cloudflare (only the on-box scheduler on 127.0.0.1 has no CF-* headers), then a constant-time
        // shared-secret check — both before any work. A no-op JSON while the streaming switch is OFF.
        [Route("funding/pump")]
        [HttpPost]
        public HttpResponseMessage fundingPump()
        {
            if (!string.IsNullOrEmpty(Header("CF-Connecting-IP")) || !string.IsNullOrEmpty(Header("CF-Ray")))
                return Request.CreateResponse(HttpStatusCode.NotFound);
            ...
            string summary = sr.RunCardFundingPump();
            HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.OK);
            response.Content = new StringContent(summary ?? "{}", Encoding.UTF8, "application/json");
            return response;
        }
