...
namespace QryptoCard.API.Filters
{
    // Per-IP rate-limit attribute for Web API actions. Applied at class or method scope; runs
    // in OnActionExecuting (before the action body). Claims from QryptoCard.Sec.RateLimiter
    // with a bucket key of "ip:<client-ip>:<route>" so different endpoints don't share quota.
    //
    ...
    //
    // Deploy context (Cloudflare topology): public ingress is via Cloudflare Tunnel, with
    // cloudflared on the IIS VM forwarding to the origin over loopback. The bucket-key client
    // IP is the value Cloudflare puts in CF-Connecting-IP, which Cloudflare overwrites on every
    // forwarded request, so it cannot be spoofed by the client. XFF is honoured only when the
    // source peer is loopback.
    //
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            if (!QryptoCard.Sec.RateLimiter.TryClaim(key, Limit, WindowSeconds))
            {
                ...
                actionContext.Response = actionContext.Request.CreateResponse(
                    (HttpStatusCode)429,
                    op);
                // Retry-After is a soft hint — the caller can retry after the window,
                // conservatively. Not the exact sliding-window tail.
